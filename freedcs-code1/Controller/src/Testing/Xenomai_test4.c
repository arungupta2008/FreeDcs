...
void demo(void *arg) {
    ...
    //rt_printf("Task name: %s \n", curtaskinfo.name);
    rt_printf("Task Value is: %d \n", num);
}
void demo1(void *arg, void *arg1) {
    RT_TASK *curtask;
    int num = * (int *)arg;
    RT_TASK_INFO curtaskinfo;
    curtask=rt_task_self();
...
int main(void)
{
  ...
  sprintf(str,"hello2\n");
  rt_printf("Task 2\n");
  rt_task_create(&demo_task2, str, 0, 50, 98);
  rt_printf("Task 3\n");
  sprintf(str,"hello3\n");
  rt_task_create(&demo_task3, str, 0, 50, 4);
  rt_printf("Task 4\n");
  sprintf(str,"hello\n");
  rt_task_create(&demo_task4, str, 0, 50, 2);
  sprintf(str,"hello5\n");
  rt_printf("Task 5\n");
  rt_task_create(&demo_task5, str, 0, 50, 1);
 /*
 sprintf(str,"hello1\n");
 rt_printf("Task 1\n");
  rt_task_create(&demo_task1, str, 0, 50, 0);
  ...
  */
   ...
   */
 int i=1;
  rt_task_start(&demo_task1, &demo1, &i);
  i =2;
  rt_task_start(&demo_task2, &demo, &i);
  i =3;
  rt_task_start(&demo_task3, &demo, &i);
  i=4;
  rt_task_start(&demo_task4, &demo, &i);
  i=5;
  rt_task_start(&demo_task5, &demo, &i);
